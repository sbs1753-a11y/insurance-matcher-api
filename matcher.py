...
MATCHING_RULES = {
    ...
    # B형 별칭: "X종 수술비" → "X종수술비"
    "1종수술비": {"type": "surgery_grade", "grade": 1},
    "2종수술비": {"type": "surgery_grade", "grade": 2},
    "3종수술비": {"type": "surgery_grade", "grade": 3},
    "4종수술비": {"type": "surgery_grade", "grade": 4},
    "5종수술비": {"type": "surgery_grade", "grade": 5},
    "6종수술비": {"type": "surgery_grade", "grade": 6},
    "7종수술비": {"type": "surgery_grade", "grade": 7},

    # 암
    "암진단(일반암)": {"type": "aggregate", "key": "암진단(일반암)"},
    # B형 별칭: "일반 암 진단비" → "일반암진단비"
    "일반암진단비": {"type": "aggregate", "key": "암진단(일반암)"},
    ...
    # B형 별칭: "소액 암 진단비" → "소액암진단비"
    "소액암진단비": {"type": "direct_exclude", "keywords": [
        "유사암진단비", "소액암진단비", "유사암진단",
        "소액암진단", "소액암New보장", "소액암보장"
    ], "exclude": ["제외"]},
    "전이암진단비": {"type": "aggregate", "key": "전이암진단비"},
    "암주요치료비": {"type": "aggregate", "key": "암주요치료비"},
    "항암방사선약물치료비": {"type": "aggregate", "key": "항암방사선약물치료비"},
    # B형 별칭: "항암 방사선/약물 치료비" → "항암방사선/약물치료비"
    "항암방사선/약물치료비": {"type": "aggregate", "key": "항암방사선약물치료비"},
    "표적항암약물치료비": {"type": "aggregate", "key": "표적항암약물치료비"},
    # B형 별칭: "표적 항암 치료비" → "표적항암치료비"
    "표적항암치료비": {"type": "aggregate", "key": "표적항암약물치료비"},
    ...
    # B형 별칭: "면역 항암 치료비" → "면역항암치료비"
    "면역항암치료비": {"type": "direct", "keywords": [
        "면역항암약물치료비", "특정면역항암약물허가치료",
        "면역항암약물허가치료", "특정면역항암",
        "카티항암약물허가치료", "카티(CAR-T)항암약물허가치료",
    ]},
    "카티항암약물치료비": {"type": "direct", "keywords": [
        "카티항암약물치료비", "카티항암약물허가치료", "CAR-T"
    ]},
    # B형 별칭: "중입자 항암 치료비" → "중입자항암치료비"
    "중입자항암치료비": {"type": "direct", "keywords": [
        "중입자항암치료비", "중입자치료비", "중입자방사선치료",
        "중입자선치료", "항암중입자",
    ]},
    ...
    # B형 별칭: "다빈치로봇 암수술비" → "다빈치로봇암수술비"
    "다빈치로봇암수술비": {"type": "direct_exclude", "keywords": [
        "다빈치로봇수술비", "다빈치로봇수술",
        "암다빈치로봇수술", "로봇암(특정암제외)수술",
        "로봇암수술", "로봇수술", "로봇암수술(다빈치",
    ], "exclude": ["종합병원", "주요치료", "소액암"]},
    # B형: "암 수술비" → "암수술비" (일반 암수술비 매칭)
    "암수술비": {"type": "direct_exclude", "keywords": [
        "암수술비", "암수술", "일반암수술",
    ], "exclude": ["소액암", "유사암", "로봇", "다빈치"]},

    # 뇌
    "뇌혈관질환진단비": {"type": "aggregate", "key": "뇌혈관질환진단비"},
    "뇌혈관질환": {"type": "aggregate", "key": "뇌혈관질환진단비"},
    # B형 별칭: "뇌혈관 진단비" → "뇌혈관진단비"
    "뇌혈관진단비": {"type": "aggregate", "key": "뇌혈관질환진단비"},
    "뇌혈수술비": {"type": "aggregate", "key": "뇌혈수술비"},
    "뇌혈관질환수술비": {"type": "aggregate", "key": "뇌혈관질환수술비"},
    # B형 별칭: "뇌혈관 수술비" → "뇌혈관수술비"
    "뇌혈관수술비": {"type": "aggregate", "key": "뇌혈관질환수술비"},
    ...
    # B형 별칭: "뇌졸중 진단비" → "뇌졸중진단비"
    "뇌졸중진단비": {"type": "direct", "keywords": [
        "뇌졸중진단비", "뇌졸증진단비",
        "뇌혈관질환(2)진단",
    ]},
    "뇌출혈진단비": {"type": "direct_exclude", "keywords": ["뇌출혈진단비", "뇌출혈진단"], "exclude": ["외상성", "주요상해"]},
    "뇌출혈": {"type": "direct_exclude", "keywords": ["뇌출혈진단비", "뇌출혈진단"], "exclude": ["외상성", "주요상해"]},
    ...
    # B형 별칭: "심혈관 진단비" → "심혈관진단비"
    "심혈관진단비": {"type": "direct", "keywords": [
        "심혈관질환진단비", "기타부정맥진단", "부정맥진단",
        "심혈관질환(특정", "심혈관질환(주요심장염증)",
    ]},
    "허혈성심장질환진단비": {"type": "aggregate", "key": "허혈성심장질환진단비"},
    "허혈성심장질환": {"type": "aggregate", "key": "허혈성심장질환진단비"},
    # B형 별칭: "허혈성 진단비" → "허혈성진단비"
    "허혈성진단비": {"type": "aggregate", "key": "허혈성심장질환진단비"},
    "허혈성심장질환수술비": {"type": "aggregate", "key": "허혈성심장질환수술비"},
    # B형 별칭: "심장 수술비" → "심장수술비" (= 허혈성심장질환수술비)
    "심장수술비": {"type": "aggregate", "key": "허혈성심장질환수술비"},
    ...
    # B형 신규: 혈전용해치료비
    "혈전용해치료비": {"type": "direct", "keywords": [
        "혈전용해치료비", "혈전용해치료", "혈전용해",
    ]},
    # B형 신규: 종합병원중환자실치료비
    "종합병원중환자실치료비": {"type": "direct", "keywords": [
        "종합병원중환자실치료비", "중환자실치료비", "종합병원중환자실",
        "중환자실입원일당",
    ]},

    # 입원/응급
    "질병입원": {"type": "direct_exclude", "keywords": ["질병입원일당", "질병입원비"], "exclude": ["다빈도", "특정", "수술", "중환자실", "상급병실", "간병인", "간호간병", "요양성", "2-3인실", "1인실", "종합병원", "상급종합병원", "2,3인실", "특실"]},
    # B형 별칭: "질병 입원일당" → "질병입원일당"
    "질병입원일당": {"type": "direct_exclude", "keywords": ["질병입원일당", "질병입원비"], "exclude": ["다빈도", "특정", "수술", "중환자실", "상급병실", "간병인", "간호간병", "요양성", "2-3인실", "1인실", "종합병원", "상급종합병원", "2,3인실", "특실"]},
    "상해입원": {"type": "direct_exclude", "keywords": ["상해입원일당", "상해입원비"], "exclude": ["수술", "중환자실", "상급병실", "간병인", "간호간병", "2-3인실", "1인실", "종합병원", "상급종합병원", "2,3인실", "특실"]},
    # B형 별칭: "상해 입원일당" → "상해입원일당"
    "상해입원일당": {"type": "direct_exclude", "keywords": ["상해입원일당", "상해입원비"], "exclude": ["수술", "중환자실", "상급병실", "간병인", "간호간병", "2-3인실", "1인실", "종합병원", "상급종합병원", "2,3인실", "특실"]},
    ...
    "간호간병통합일당": {"type": "direct", "keywords": [
        "간호간병통합일당", "간호간병일당", "간호간병",
    ]},
    ...
    # B형 별칭: "상해 사망" → "상해사망"
    "상해사망": {"type": "aggregate", "key": "상해사망/재해사망"},

    # 후유장해
    # 신한라이프: 재해장해특약 7000만원 + 주계약(신한통합건강보험) 500만원 = 7500만원
    "상해후유장해3%": {"type": "aggregate", "key": "상해후유장해3%"},
    # B형 별칭: "상해 후유장해 (3% 이상)" → "상해후유장해(3%이상)"
    "상해후유장해(3%이상)": {"type": "aggregate", "key": "상해후유장해3%"},
    ...
    # B형 별칭: "질병 후유장해 (3% 이상)" → "질병후유장해(3%이상)"
    "질병후유장해(3%이상)": {"type": "direct_exclude", "keywords": [
        "질병3%이상후유장해", "질병후유장해3%",
        "질병후유장해보장", "질병후유장해(3-100%)",
        "질병후유장해", "질병장해"
    ], "exclude": ["80%이상"]},
    ...
    # B형 별칭: "골절 진단비" → "골절진단비"
    "골절진단비": {"type": "aggregate", "key": "골절진단"},
    "골절수술": {"type": "aggregate", "key": "골절수술비"},
    # B형 별칭: "골절 수술비" → "골절수술비"
    "골절수술비": {"type": "aggregate", "key": "골절수술비"},
    "깁스": {"type": "direct", "keywords": ["깁스치료", "깁스"]},
    # B형 별칭: "깁스 치료비" → "깁스치료비"
    "깁스치료비": {"type": "direct", "keywords": ["깁스치료", "깁스"]},

    # 치매
    "경도치매": {"type": "direct", "keywords": ["경도치매"]},
    # B형 별칭: "경도 진단비" → "경도진단비" (경도치매 진단비)
    "경도진단비": {"type": "direct", "keywords": ["경도치매"]},
    "중증도치매": {"type": "direct", "keywords": ["중증도치매", "중등도치매"]},
    "중증치매": {"type": "direct", "keywords": ["중증치매진단비", "중증치매"]},
    # B형 별칭: "중증 진단비" → "중증진단비" (중증치매 진단비)
    "중증진단비": {"type": "direct", "keywords": ["중증치매진단비", "중증치매"]},
    "중증치매생활비": {"type": "direct", "keywords": ["중증치매생활비"]},
    # B형 별칭: "경증 생활비" → "경증생활비" (경도/중증치매 생활비)
    "경증생활비": {"type": "direct", "keywords": ["중증치매생활비", "경도치매생활비", "치매생활비"]},

    # 운전자
    "벌금": {"type": "direct_exclude", "keywords": ["자동차사고벌금", "벌금"], "exclude": ["대물"]},
    # B형 별칭: "벌금 ( 대인 )" → "벌금(대인)"
    "벌금(대인)": {"type": "direct_exclude", "keywords": ["자동차사고벌금", "벌금"], "exclude": ["대물"]},
    ...
    # B형 별칭: "자동차 사고 부상 치료비" → "자동차사고부상치료비"
    "자동차사고부상치료비": {"type": "aggregate", "key": "자동차사고부상14등급"},

    # 배상책임
    "가족일상배상책임": {"type": "aggregate", "key": "가족일상배상책임"},
    # B형 별칭: "가족 일상생활배상책임" → "가족일상생활배상책임"
    "가족일상생활배상책임": {"type": "aggregate", "key": "가족일상배상책임"},

    # B형 신규: 화재 벌금
    "화재벌금": {"type": "direct", "keywords": ["화재벌금", "화재배상"]},
}
